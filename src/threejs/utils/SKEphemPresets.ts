import { SKEphem, GM } from './SKEphem';
import { kmToAu } from './SKUnits';
import { naturalSatellites } from '../data/natural-satellites';
...
export class NaturalSatellites {
  private _satellitesByPlanet: any = {};
    ...
    moons.forEach(moon => {
      ...
      switch (moon.Planet) {
        ...
        case 'Pluto':
          ...
          break;
        default:
          ephemGM = (GM as any)[moon.Planet.toUpperCase()];
      }
      if (!ephemGM) {
        console.error(`Could not look up GM for ${moon.Planet}`);
      }

      const ephem = new SKEphem(
        {
          GM: ephemGM,
          epoch: moon['Epoch JD'],
          a: kmToAu(moon.a),
